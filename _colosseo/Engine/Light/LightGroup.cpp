...
void LightGroup::SetDefaultAmbientColor(const XMFLOAT3 &color)
{
	DefaultAmbientColor = color;
	DefaultDirty = true;
}

void LightGroup::SetDefaultDirLightActive(bool active)
{
	DefaultDirLight.SetActive(active);
	DefaultDirty = true;
}

void LightGroup::SetDefaultDirLightDir(const XMVECTOR &lightdir)
{
	DefaultDirLight.SetLightDir(lightdir);
	DefaultDirty = true;
}

void LightGroup::SetDefaultDirLightColor(const XMFLOAT3 &lightcolor)
{
	DefaultDirLight.SetLightColor(lightcolor);
	DefaultDirty = true;
}

void LightGroup::DefaultDraw(ID3D12GraphicsCommandList *cmdList, UINT rootParameterIndex)
{
	//定数バッファビューをセット
	cmdList->SetGraphicsRootConstantBufferView(rootParameterIndex, DefaultConstBuff->GetGPUVirtualAddress());
}

void LightGroup::DefaultTransferConstBuffer()
{
	HRESULT result;
	//定数バッファへデータ転送
	ConstBufferData *constMap = nullptr;
	result = DefaultConstBuff->Map(0, nullptr, (void **)&constMap);
	if (SUCCEEDED(result)) {
		//環境光
		constMap->ambientColor = DefaultAmbientColor;
		if (DefaultDirLight.IsActive()) {
			constMap->dirLights[0].active = 1;
			constMap->dirLights[0].lightv = -DefaultDirLight.GetLightDir();
			constMap->dirLights[0].lightcolor = DefaultDirLight.GetLightColor();
		}
		//ライトが無効ならライト色を0に
		else {
			constMap->dirLights[0].active = 0;
		}
		//ここからその他の光(空き埋め用)
		//平行光源
		for (int i = 1; i < DirLightNum; i++) {
			constMap->dirLights[i].active = 0;
		}
		//点光源
		for (int i = 0; i < PointLightNum; i++) {
		...
		DefaultConstBuff->Unmap(0, nullptr);
	}
}
...
void LightGroup::DefaultLightSetting()
{
	...
	DefaultAmbientColor = { 1,1,1 };
	DefaultDirLight.SetActive(true);
	DefaultDirLight.SetLightColor({ 1.0f, 1.0f, 1.0f });
	DefaultDirLight.SetLightDir({ 0.0f, 1.0f, 0.0f, 0 });
}

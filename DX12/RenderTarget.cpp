...
void RenderTarget::Initialize(DXGI_FORMAT RTV_Format, bool IsUseDepth)
{
	...
	// RTV用テクスチャ生成
	m_RTV_Texture = TexManager::GetPostTexture(WINDOW_WIDTH, WINDOW_HEIGHT, XMFLOAT4(255.0f, 255.0f, 255.0f, 255.0f), RTV_Format);
	// RTV用デスクリプタヒープ設定
	...
	if (m_IsUseDepth) {
		// DSV用テクスチャ生成
		m_DSV_Texture = TexManager::GetPostDepthTexture(WINDOW_WIDTH, WINDOW_HEIGHT, XMFLOAT4(255.0f, 255.0f, 255.0f, 255.0f));
		// DSV用デスクリプタヒープ設定

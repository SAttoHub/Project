...
bool Collision2::CheckCircleCircle(const Circle &circleA, const Circle &circleB)
{
...
bool Collision2::CheckCircleBox(const Circle &circle, const Box &box)
{
	float L = box.pos1.x;
	float R = box.pos2.x;
	float T = box.pos1.y;
	...
	if (L - circle.radius > x || R + circle.radius < x || T - circle.radius > y || B + circle.radius < y) {//矩形に円の半径分を足した範囲
		return false;
	}
	if (L > x && T > y && !((L - x) * (L - x) + (T - y) * (T - y) < circle.radius * circle.radius)) {//左上の当たり判定
		return false;
	}
	if (R < x && T > y && !((R - x) * (R - x) + (T - y) * (T - y) < circle.radius * circle.radius)) {//右上の当たり判定
		return false;
	}
	if (L > x && B < y && !((L - x) * (L - x) + (B - y) * (B - y) < circle.radius * circle.radius)) {//左下の当たり判定
		return false;
	}
	if (R < x && B < y && !((R - x) * (R - x) + (B - y) * (B - y) < circle.radius * circle.radius)) {//右下の当たり判定
		return false;
	}
	return true;
}

bool Collision2::CheckBoxBox(const Box &boxA, const Box &boxB)
{
	float L = boxA.pos1.x;
	float R = boxA.pos2.x;
	float T = boxA.pos1.y;
	float B = boxA.pos2.y;
	float L2 = boxB.pos1.x;
	float R2 = boxB.pos2.x;
	float T2 = boxB.pos1.y;
	float B2 = boxB.pos2.y;
	if ((R > L2) &&
		(L < R2))
	{
		if ((B > T2) &&
...
bool Collision2::CheckSphere2Sphere(const Sphere &sphereA, const Sphere &sphereB, DirectX::XMVECTOR *inter)
{
...
bool Collision2::CheckSphere2Plane(const Sphere &sphere, const Plane &plane, DirectX::XMVECTOR *inter)
{
...
bool Collision2::CheckSphere2Triangle(const Sphere &sphere, const Triangle &triangle, DirectX::XMVECTOR *inter)
{
	XMVECTOR p;
...
bool Collision2::CheckRay2Plane(const Ray &lay, const Plane &plane, float *distance, DirectX::XMVECTOR *inter)
{
...
bool Collision2::CheckRay2Triangle(const Ray &lay, const Triangle &triangle, float *distance, DirectX::XMVECTOR *inter)
{
	// 三角形が乗っている平面を算出
	Plane plane;
	XMVECTOR interPlane;
...
bool Collision2::CheckRay2Sphere(const Ray &lay, const Sphere &sphere, float *distance, DirectX::XMVECTOR *inter)
{

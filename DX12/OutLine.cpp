...
void OutLine::SetupGraphPrimitive()
{
	HRESULT result;
...
#pragma endregion
	...
	LoadShader(&Graph_psBlob, L"Resource/shader/OutLinePS.hlsl", "main", "ps_5_0");
...
void OutLine::LoadShader(ID3DBlob **blob, LPCWSTR FileName, LPCSTR EntryPointName, LPCSTR ModelName)
{
	HRESULT result;
	//シェーダーの読み込みとコンパイル
...
void OutLine::Initialize(bool isUse) {
	if (isUse) {
		UseFlag = 1.0f;
	}
	else {
...
void OutLine::SetState(bool isUse)
{
	if (isUse) {
		UseFlag = 1.0f;
	}
...
void OutLine::Draw(int TexNum1, int TexNum2)
{
	int Num = 0;
	...
	if (SUCCEEDED(ConstBuff0->Map(0, nullptr, (void **)&constMap0))) {
		...
		constMap0->Camera_viewproj_inv = ShadowMapLight::matViewProjectionInverseCamera;

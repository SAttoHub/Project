...
void PostEffect::SetupGraphPrimitive(DXGI_FORMAT format)
{
	HRESULT result;
...
#pragma endregion
	...
	LoadShader(&Graph_psBlob, L"Resource/shader/PostEffectPS.hlsl", "main", "ps_5_0");
...
void PostEffect::LoadShader(ID3DBlob **blob, LPCWSTR FileName, LPCSTR EntryPointName, LPCSTR ModelName)
{
	HRESULT result;
	//シェーダーの読み込みとコンパイル
...
void PostEffect::Initialize(DXGI_FORMAT format) {
	...
	//);
	TexNum = TexManager::GetPostTexture(WINDOW_WIDTH, WINDOW_HEIGHT, XMFLOAT4(255.0f, 255.0f, 255.0f, 255.0f), format);
...
void PostEffect::Draw(int TexNum1)
{
	int Num = 0;
	...
	Timer += 1;
	ConstBufferDataTime *constMap1 = nullptr;
	if (SUCCEEDED(ConstBuffTime->Map(0, nullptr, (void **)&constMap1))) {
	...
	for (int i = 0; i < activeCount; i++) {
		...
		//DirectXBase::cmdList->SetDescriptorHeaps(1, descHeapSRV.GetAddressOf());
...
void PostEffect::ClearDepth()
{
}

void PostEffect::PreDrawScene(int Num)
{
	//リソースバリアを変更

...
//二点間の距離を求める
static inline float Distance2D(const DirectX::XMFLOAT2 &v1, const DirectX::XMFLOAT2 &v2) {
	return sqrtf(powf(v1.x - v2.x, 2.0f) + powf(v1.y - v2.y, 2.0f));
}
//二点間の距離を求める
static inline float Distance3D(const DirectX::XMFLOAT3 &v1, const DirectX::XMFLOAT3 &v2) {
	return sqrtf(powf(v1.x - v2.x, 2.0f) + powf(v1.y - v2.y, 2.0f) + powf(v1.z - v2.z, 2.0f));
}

//内積を求める
static inline float Dot2D(const DirectX::XMFLOAT2 &v1, const DirectX::XMFLOAT2 &v2) {
	return v1.x * v2.x + v1.y * v2.y;
}
//内積を求める
static inline float Dot3D(const DirectX::XMFLOAT3 &v1, const DirectX::XMFLOAT3 &v2) {
	return v1.x * v2.x + v1.y * v2.y + v1.z * v2.z;
}

//外積を求める
static inline float Cross2D(const DirectX::XMFLOAT2 &v1, const DirectX::XMFLOAT2 &v2) {
	return v1.x * v2.y - v1.y * v2.x;
}
//外積を求める
static inline DirectX::XMFLOAT3 Cross3D(const DirectX::XMFLOAT3 &v1, const DirectX::XMFLOAT3 &v2) {
...
//ベクトルの長さを求める
static inline float Length2D(const DirectX::XMFLOAT2 &v2) {
	return sqrtf(Dot2D(v2, v2));
}
//ベクトルの長さを求める
static inline float Length3D(const DirectX::XMFLOAT3 &v2) {
	return sqrtf(Dot3D(v2, v2));
}

//角度を求める
static inline float CalAngle3D(const DirectX::XMFLOAT3 &v1, const DirectX::XMFLOAT3 &v2) {
	return Dot3D(v1, v2) / (Length3D(v1) * Length3D(v2));
}

//ベクトルを正規化する
static inline DirectX::XMFLOAT2 Normalize2D(const DirectX::XMFLOAT2 &v2) {
...
//ベクトルを正規化する
static inline DirectX::XMFLOAT3 Normalize3D(const DirectX::XMFLOAT3 &v2) {

...
#pragma endregion
	...
	LoadShader(&Graph_gsBlob, L"Resource/shader/DepthOfFieldGS.hlsl", "main", "gs_5_0");
	LoadShader(&Graph_psBlob, L"Resource/shader/DepthOfFieldPS.hlsl", "main", "ps_5_0");
...
void DepthOfField::LoadShader(ID3DBlob **blob, LPCWSTR FileName, LPCSTR EntryPointName, LPCSTR ModelName)
{
	HRESULT result;
	//シェーダーの読み込みとコンパイル
...
void DepthOfField::Draw(int TexNum1, int TexNum2, int TexNum3, int TexNum4)
{
	int Num = 0;
	...
	for (int i = 0; i < activeCount; i++) {
		...
		DirectXBase::cmdList->SetGraphicsRootDescriptorTable(3, CD3DX12_GPU_DESCRIPTOR_HANDLE(
			...
			TexNum3,
			DirectXBase::dev->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV)));
		DirectXBase::cmdList->SetGraphicsRootDescriptorTable(4, CD3DX12_GPU_DESCRIPTOR_HANDLE(
			TexManager::TextureDescHeap->GetGPUDescriptorHandleForHeapStart(),
			TexNum4,
			DirectXBase::dev->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV)));
		//描画コマンド
...
void DepthOfField::ClearDepth()
{
}

void DepthOfField::PreDrawScene()
{
	//リソースバリアを変更
...
void DepthOfField::PostDrawScene()
{
